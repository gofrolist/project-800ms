...
- [x] T011 [P] Populate `services/retriever/tests/conftest.py` with `pgvector_postgres` testcontainers fixture (uses `pgvector/pgvector:pg16` image; runs `apps/api` Alembic migrations against the container in session-scoped setup); exports `db_session` async fixture for per-test rollback isolation
