...
6. **`flash-attn` install made non-fatal at Docker build time** (reliability) — `pip install flash-attn --no-build-isolation` fails with `metadata-generation-failed` on GCP L4 + CUDA 12.6 runtime base image; its egg_info step can't locate the packaging/torch combo it expects. Our default backend is `official` which iterates `flash_attention_2 → sdpa → eager` and falls back cleanly when flash_attn is absent at import time, so build-failing on this optional install would block a runtime that doesn't need it. Patched `Dockerfile:20` to suffix `|| echo "WARNING: ..."` on the install — fast path stays fast when the wheel builds; a broken build environment doesn't block deploys.
7. **Streaming exposed on the `official` backend** (performance) — upstream's `OfficialQwen3TTSBackend` only implements `generate_speech` and `generate_voice_clone` (both non-streaming). The router's `stream=True` path calls `backend.generate_speech_streaming(...)` (CustomVoice) and `backend.generate_voice_clone_streaming(...)` (voice-clone profiles), which on `official` raised `AttributeError` at request time. The underlying `Qwen3TTSModel` already supports streaming via `stream_generate_custom_voice` (`qwen_tts/inference/qwen3_tts_model.py:729`) and `stream_generate_voice_clone` (`:525`); we added thin `async generate_speech_streaming` and `async generate_voice_clone_streaming` wrappers that pull chunks on the default executor so the asyncio loop stays responsive. The `optimized` backend already had its own streaming path; only `official` was unpatched.
