...
7. **Streaming exposed on the `official` backend** (performance) — upstream's `OfficialQwen3TTSBackend` only implements `generate_speech` (non-streaming). The router's `stream=True` path calls `backend.generate_speech_streaming(...)`, which on `official` raised `AttributeError` at request time. The underlying `Qwen3TTSModel` already supports streaming via `stream_generate_custom_voice` (see `qwen_tts/inference/qwen3_tts_model.py:729`); we added a thin `async generate_speech_streaming` on the backend that wraps the generator and pulls chunks on the default executor so the asyncio loop stays responsive. Drops TTFB from ~20s to ~1-2s on our L4 with the 1.7B-CustomVoice variant. The `optimized` backend already had its own streaming path; only `official` was unpatched.
